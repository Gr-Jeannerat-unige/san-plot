function [ where_cut_stat ] = determine_position_measure_SNR(work_sp,opt,plot_results,magnitude_mode)
if nargin<4
    magnitude_mode=0;
end
...
for cur_value=test_list_of_values_where_cut_stat
    val=(work_sp(round(size(work_sp,1)*cur_value),1))/(-simple_norminv((cur_value)/2,magnitude_mode));%level of signal at half the distribution of pos signals
