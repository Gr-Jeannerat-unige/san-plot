function this_return=plot_2d_interp(dataset,hold_me,scales_in,sizes_in,fig_number)
%%
...
if ~isfield(dataset,'scale1')
    figure(fig_number);clf
    this_return=plot(dataset.scale2,dataset.spectrum);
    set(gca,'XDir','reverse');
    if nargin<3
        scales_in=[min(dataset.scale2) max(dataset.scale2) ];
    end
    return
end
...
if nargin<3
    scales_in=[min(dataset.scale2) max(dataset.scale2) min(dataset.scale1) max(dataset.scale1)];
end
if nargin<2
    hold_me=0;
end
...
if interpolate
    ...
    %use interpft
    nbpt2=size(dataset.spectrum,2)*abs(scales_in(1,2)-scales_in(1,1))/abs(min(dataset.scale2)-max(dataset.scale2));
    nbpt1=size(dataset.spectrum,1)*abs(scales_in(1,3)-scales_in(1,4))/abs(min(dataset.scale1)-max(dataset.scale1));
    ...
    data=interp2(dataset.scale2,dataset.scale1,dataset.spectrum,mgr2, mgr1,'nearest')   ;
    ...
    int_scale2=dataset.scale2;
    int_scale1=dataset.scale1;
    data=dataset.spectrum;
    
end
if ~isfield(dataset,'cont_level_list')
[cont_level_list]=generate_contour_level_list(5*dataset.noise_level,max(max(dataset.spectrum)),2);
else
    cont_level_list= dataset.cont_level_list;
end
